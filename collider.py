class Collider:
    ...
    def collide(self,collider,velocity):
        ...
        x_entry = (collider.x1 - self.x2 if vx > 0 else collider.x2 - self.x1)
        x_exit  = (collider.x2 - self.x1 if vx > 0 else collider.x1 - self.x2)

        y_entry = (collider.y1 - self.y2 if vy > 0 else collider.y2 - self.y1)
        y_exit  = (collider.y2 - self.y1 if vy > 0 else collider.y1 - self.y2)

        z_entry = (collider.z1 - self.z2 if vz > 0 else collider.z2 - self.z1)
        z_exit  = (collider.z2 - self.z1 if vz > 0 else collider.z1 - self.z2)
